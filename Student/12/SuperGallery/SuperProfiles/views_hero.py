    ...
    def get_context_data(self, **kwargs):
        ...
        def carousel_data():
            ...
            for hero in Hero.objects.all():
                if(hero.photo):
                    data.append(dict(image_url=f"/media/{hero.photo.image}", label=hero.title, link="/hero/"+str(hero.pk), active=""))
                else:
                    data.append(dict(image_url="", label=hero.title, link="/hero/"+str(hero.pk), active=""))
            if data:
