...
from .models import Hero, Article, Reporter, Photo, Note
...
class IsPhotoCreatorMixin(AccessMixin):
    def test_func(self):
        photo = get_object_or_404(Photo, pk = self.kwargs["pk"])
        return self.request.user == photo.reporter.user
...
class IsNoteCreatorMixin(AccessMixin):
    def test_func(self):
        note = get_object_or_404(Note, pk = self.kwargs["pk"])
        return self.request.user == note.reporter.user
